...
inline void label(Span<PlacedChar> placed_chars, SizedFont *font, DrawTextParams params = {}) {
...
inline void label(Span<utf8> string, DrawTextParams params = {}) {
	if (string.size == 0)
		return;
...
inline bool button_base(umm id, ButtonTheme const &theme, std::source_location location) {
	...
	state.press_t       = lerp<f32>(state.press_t,      (f32)mouse_held(0),     shared_data->frame_time * theme.press_speed);
	state.hover_stay_t  = lerp<f32>(state.hover_stay_t, (f32)currently_hovered, shared_data->frame_time * theme.hover_stay_speed);
	...
	state.click_t       = lerp<f32>(state.click_t,       0, shared_data->frame_time * theme.click_speed);
	state.hover_enter_t = lerp<f32>(state.hover_enter_t, 0, shared_data->frame_time * theme.hover_enter_speed);
...
bool input_field(InputFieldCallbacks callbacks, auto &state, auto &value, auto &theme) {
	...
	if (state.editing) {
		...
		state.caret_blink_time += shared_data->frame_time;
...
inline void draw_property(Span<utf8> name, f32 &value, u64 id, std::source_location location) {
	tg::Viewport line_viewport = current_viewport;
	line_viewport.min.y = current_viewport.max.y - line_height - current_property_y;
	line_viewport.max.y = line_viewport.min.y + line_height;
...
inline void draw_property(Span<utf8> name, v3f &value, u64 id, std::source_location location) {
	header(name);
...
inline void draw_property(Span<utf8> name, quaternion &value, u64 id, std::source_location location) {
	v3f angles = degrees(to_euler_angles(value));
	draw_property(name, angles, id,location);
	value = quaternion_from_euler(radians(angles));
}

inline void draw_property(Span<utf8> name, List<utf8> &value, u64 id, std::source_location location) {
	header(name);
...
inline void draw_property(Span<utf8> name, Texture2D *&value, u64 id, std::source_location location) {
...
inline void draw_property(Span<utf8> name, Mesh *&value, u64 id, std::source_location location) {
	Span<utf8> extensions[] = {
		u8"idk"s
	};
	draw_asset_property(name, value ? value->name : u8"null"s, id, location, extensions, [&] (Span<utf8> path) {
