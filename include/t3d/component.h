...
extern "C" void register_component_##ComponentT(HashMap<Span<utf8>, ComponentUID> &component_name_to_uid, HashMap<ComponentUID, ComponentInfo> &component_infos, ComponentUID (*get_new_uid)()) { \
	auto name = u8#ComponentT##s;\
	auto found_uid = component_name_to_uid.find(name); \
 \
	ComponentUID uid = found_uid ? *found_uid : get_new_uid(); \
 \
	auto &info = get_component_infos().get_or_insert(uid); \
 \
	info.name = name; \
 \
 ...
 \
	...
	info.draw_properties    = PROPERTY_DRAWER_ADAPTOR(ComponentT); \
