...
void main() {
	...
	void render(tg::RenderTarget *source, tg::RenderTarget *destination) {
		...
		tg::set_shader(shared_data->blit_texture_color_shader);
		tg::update_shader_constants(shared_data->blit_texture_color_constants, {.color = V4f(intensity)});
		tg::set_shader_constants(shared_data->blit_texture_color_constants, 0);
