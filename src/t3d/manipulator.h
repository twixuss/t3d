...
inline bool operator==(ManipulatorStateKey const &a, ManipulatorStateKey const &b) {
...
inline ManipulatedTransform manipulate_transform(v3f position, quaternion rotation, v3f scale, ManipulateKind kind, u32 id = 0, std::source_location source_location = std::source_location::current()) {
	...
	auto camera = app->current_camera;
	auto camera_entity = app->current_camera_entity;

	auto &state = manipulator_states.get_or_insert({.id = id, .camera = camera, .location = source_location});
	...
	auto camera_forward = camera_entity->rotation * v3f{0,0,-1};
	auto camera_forward_dot_handle_direction = dot(position - camera_entity->position, camera_forward);
	f32 handle_size_scaled_by_distance = handle_size * distance(position, camera_entity->position);

	auto mouse_position = editor->get_mouse_position_in_current_viewport();
	...
	switch (kind) {
		...
		case Manipulate_scale: {
			//m4 handle_matrix = m4::translation(position) * m4::rotation_r_zxy(rotation) * m4::scale(handle_size * dot(position - camera_entity.position, m4::rotation_r_zxy(camera_entity.rotation) * v3f{0,0,-1}));
			m4 handle_matrix = m4::translation(position) * (m4)rotation * m4::scale(handle_size_scaled_by_distance);

			v3f handle_viewport_position = camera->world_to_window(handle_matrix * v4f{0,0,0,1});
			...
			Array<v2f, 3> handle_viewport_axis_tips = {
				camera->world_to_window(handle_world_axis_tips[0]).xy,
				camera->world_to_window(handle_world_axis_tips[1]).xy,
				camera->world_to_window(handle_world_axis_tips[2]).xy
			};

			Array<v2f, 12> handle_viewport_plane_points = {
				camera->world_to_window((handle_matrix * v4f{0,   0.4f,0.4f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0,   0.4f,0.8f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0,   0.8f,0.4f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0,   0.8f,0.8f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.4f,0,   0.4f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.4f,0,   0.8f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.8f,0,   0.4f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.8f,0,   0.8f,1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.4f,0.4f,0,   1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.4f,0.8f,0,   1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.8f,0.4f,0,   1}).xyz).xy,
				camera->world_to_window((handle_matrix * v4f{0.8f,0.8f,0,   1}).xyz).xy,
			};
			...
			if (closest_dist > app->window->client_size.y * handle_grab_thickness) {
			...
			if (state.dragging_part_index != null_manipulator_part) {
				...
				if (state.dragging_part_index < 3) {
					...
					v4f end = camera_to_world_matrix * V4f(map(closest_in_viewport, v2f{}, (v2f)editor->current_viewport.size(), v2f{-1,-1}, v2f{1,1}), 1, 1);
					...
					v4f end = camera_to_world_matrix * V4f(map((v2f)mouse_position, v2f{}, (v2f)editor->current_viewport.size(), v2f{-1,-1}, v2f{1,1}), 1, 1);
					ray<v3f> cursor_ray = ray_origin_end(camera_entity->position, end.xyz / end.w);
				...
				if (begin_drag) {
					...
					if (kind == Manipulate_position) {
						manipulated_transform.position = new_position + state.drag_offset * (dot(new_position - camera_entity->position, camera_forward) / dot(state.original_click_position - camera_entity->position, camera_forward));
					} else {
		...
		case Manipulate_rotation: {
			...
			for (u32 part_index = 0; part_index < 3; ++part_index) {
				...
				for (u32 point_index = 0; point_index < point_count; ++point_index) {
					...
					if (dot(normalize(point - position), normalize(camera_entity->position - position)) < -0.1)
						continue;

					//debug_line(position, point);

					f32 dist = distance((v2f)mouse_position, camera->world_to_window(point).xy);
					if (dist < closest_dist) {
			...
			if (closest_dist > handle_grab_thickness * editor->current_viewport.size().y) {
				closest_element = null_manipulator_part;
			}
			...
			if (state.dragging_part_index != null_manipulator_part) {
				...
				if (begin_drag) {
					v2f intersect_position_viewport = (v2f)(app->current_mouse_position - editor->current_viewport.min);
					...
					state.tangent = ray_origin_direction(
						...
						normalize((camera->world_to_window(tip).xy - intersect_position_viewport))
					);
				...
				f32 dist = pi * -2 * dot(
					...
					state.tangent.direction
				) / editor->current_viewport.size().y;
	...
	draw_request.highlighted_part_index = in_bounds(app->current_mouse_position, editor->current_viewport) ? ((state.dragging_part_index != null_manipulator_part) ? state.dragging_part_index : closest_element) : -1;
